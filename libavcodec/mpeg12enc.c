...
void ff_mpeg1_encode_picture_header(MpegEncContext *s)
{
    ...
    ret = ff_alloc_misp_precision_timestamp_sei(s->cur_pic.ptr->f, (void **) &sei_data, &sei_size);
    if (ret < 0) {
    ...
    ret = ff_alloc_sync_precision_timestamp_sei(s->cur_pic.ptr->f, (void **) &sei_data, &sei_size);
    if (ret < 0) {
